...
# ./partition-3x-citrus.sh target-disk [ / -?-u(n?mount)? / ]
...
KEYFILE="./privkeys/3x-citrus/system.key"
ZPOOL="3x-citrus"
LUKS_FORMAT=( --batch-mode luksFormat --type luks2 )

einfo "Checking for existing mounts"
for dir in /run/keystore/"$ZPOOL" /mnt/"$ZPOOL"-altroot/{boot/efi,boot,proc,sys,tmp,home,persist/cache,persist,root,var/log,var/guix,gnu/store,run/keystore/3x-citrus,}; do
  if mountpoint -q -- "$dir"; then erun "Unmounting $dir" umount "$dir"; fi
done

zfs unload-key -r 3x-citrus || true
...
erun "3x-citrus/gnu-store"     create-zfs-dataset gnu-store     gnu/store
erun "3x-citrus/home"          create-zfs-dataset home          home

erun "3x-citrus/persist"       create-zfs-dataset persist       persist
erun "Stubbing /persist/cache" stub_at_path /mnt/"$ZPOOL"-altroot/persist/cache

erun "3x-citrus/persist-cache" create-zfs-dataset persist-cache persist/cache
erun "3x-citrus/root"          create-zfs-dataset root          root
erun "3x-citrus/var-log"       create-zfs-dataset var-log       var/log
erun "3x-citrus/var-guix"      create-zfs-dataset var-guix      var/guix
